...
{
public:
  explicit TranslateDlg( QWidget* = nullptr, Qt::WindowFlags flags = nullptr, bool = true );
  ~TranslateDlg() override;
  int                   getMode() const;

    void                  addMode( int, const QString& );
  void                  clear();
...
: QFileDialog( parent, flags )
{
  ...
  if( grid )
  {
    ...
    auto* paramGroup = new QWidget( this );
...
{
  if ( !myDlg )
    myDlg = new TranslateDlg( nullptr, nullptr, true );
...
bool Translate::exportSTEP( const QString& file, const Handle(TopTools_HSequenceOfShape)& shapes )
{
    ...
    auto type = (STEPControl_StepModelType)theDlg->getMode();
    if ( type < 0 )
        return false;
